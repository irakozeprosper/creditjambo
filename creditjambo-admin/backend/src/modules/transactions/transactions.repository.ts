...
export class TransactionsRepository {
  ...
  async findAll(): Promise<Transaction[]> {
    const { rows } = await pool.query(
      ...
      t.amount, 
      t.transaction_date,
      t.source_destination_name,
      t.source_destination_account
      FROM transactions t
      JOIN savings_accounts a ON t.account_id = a.account_id
  ...
  async findAllTransactionsToday(): Promise<Transaction[]> {
    const { rows } = await pool.query(`
